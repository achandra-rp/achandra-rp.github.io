...
const KafkaKnativeAmpSetup = () => {
  return (
    <DocPage title="End-to-End Installation & Validation Guide">
      ...
      <pre><code>{`# Check EKS cluster and OIDC
...
kubectl run kafka-test --rm -it --restart=Never --image=confluentinc/cp-kafka:latest -- \\
  kafka-topics --bootstrap-server <your-kafka-broker>:9092 --list`}</code></pre>

      <h2>2 Create IRSA Role for AMP Integration</h2>
      <h3>2.1 Gather Required Information</h3>
      <pre><code>{`# Get your AWS account ID
...
echo "OIDC ID: $OIDC_ID"`}</code></pre>

      <h3>2.2 Create IAM Role and Policy</h3>
      <pre><code>{`# Create the IAM policy
...
aws iam attach-role-policy --role-name EKS-AMP-Ingest --policy-arn arn:aws:iam::$AWS_ACCOUNT_ID:policy/EKS-AMP-Ingest-Policy`}</code></pre>

      <h3>2.3 Create ServiceAccount</h3>
      <pre><code>{`# Create the monitoring namespace
...
kubectl apply -f amp-serviceaccount.yaml`}</code></pre>

      <h3>2.4 Validate IRSA Setup</h3>
      <pre><code>{`# Check ServiceAccount annotation
...
kubectl run irsa-test --rm -it --restart=Never --serviceaccount=amp-iamproxy-ingest --namespace=monitoring --image=amazon/aws-cli:latest -- \\
  sts get-caller-identity`}</code></pre>
...
helm repo update`}</code></pre>
      ...
      <pre><code>{`# Replace these values with your actual cluster name and workspace ID
...
helm upgrade --install amp-agent prometheus-community/kube-prometheus-stack \\
  ...
  --set 'prometheus.prometheusSpec.remoteWrite[0].queueConfig.capacity=5000'`}</code></pre>

      <h3>3.3 Verify Prometheus Agent Installation</h3>
      <pre><code>{`# Check if pods are running
...
kubectl get pods -n monitoring -l app.kubernetes.io/name=prometheus -o yaml | grep -A2 -B2 serviceAccount`}</code></pre>

      <h2>4 Deploy Knative ServiceMonitors</h2>
      <h3>4.1 Install Knative Monitoring Components</h3>
      <pre><code>{`# Install Knative monitoring ServiceMonitors
kubectl apply -f https://raw.githubusercontent.com/knative-extensions/monitoring/main/servicemonitor.yaml

# Verify ServiceMonitors were created
kubectl get servicemonitor -A | grep knative`}</code></pre>

      <h3>4.2 Validate Knative Metrics Endpoints</h3>
      <pre><code>{`# Check if Knative pods expose metrics
...
curl -s localhost:9090/metrics | grep -E "knative_|serving_|eventing_" | head -5`}</code></pre>

      <h2>5 Deploy kafka-exporter and ServiceMonitor</h2>
      <h3>5.1 Create Kafka Credentials Secret</h3>
      <pre><code>{`# Create secret for Kafka authentication (adjust based on your setup)
...
# Instead, use IAM roles for service accounts`}</code></pre>

      <h3>5.2 Deploy kafka-exporter</h3>
      <pre><code>{`# kafka-exporter.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: kafka-exporter
...
spec:
  ...
  namespaceSelector:
    matchNames:
      ...
      <pre><code>{`# Apply the complete kafka-exporter setup
...
kubectl get endpoints kafka-exporter -n monitoring`}</code></pre>

      <h2>6 Comprehensive kafka-exporter Testing</h2>
      <h3>6.1 Basic Connectivity Tests</h3>
      <pre><code>{`# Check if kafka-exporter pod is running
...
kubectl exec -n monitoring -l app=kafka-exporter -- curl -s localhost:9308/metrics | head -10`}</code></pre>

      <h3>6.2 Port-Forward and Web Interface Testing</h3>
      <pre><code>{`# Port-forward to access kafka-exporter web interface
...
echo "Total kafka metrics found: $(curl -s localhost:9308/metrics | grep -c "kafka_")"`}</code></pre>

      <h3>6.3 Kafka Connection Validation</h3>
      <pre><code>{`# Check if kafka-exporter can connect to Kafka brokers
...
kubectl logs -n monitoring -l app=kafka-exporter | grep -i "auth"`}</code></pre>

      <h2>7 End-to-End Pipeline Testing</h2>
      <h3>7.1 Install awscurl (Required for AMP Querying)</h3>
      <pre><code>{`# Install awscurl for querying AMP
pip install awscurl
...
awscurl --version`}</code></pre>

      <h3>7.2 Port-Forward Prometheus Agent</h3>
      <pre><code>{`# Port-forward Prometheus agent for testing
kubectl -n monitoring port-forward deploy/<PROMETHEUS_AGENT_DEPLOYMENT_NAME> 9090:9090 &`}</code></pre>

      <h3>7.3 Verify Metrics Collection</h3>
      <pre><code>{`# Check that kafka-exporter metrics are collected by Prometheus
...
curl -sG 'http://localhost:9090/api/v1/label/__name__/values' \\
     --data-urlencode 'match[]={job=~".*knative.*"}' | jq -r '.data[]' | head -5`}</code></pre>

      <h3>7.4 Verify Remote Write Queue Health</h3>
      <pre><code>{`# Check samples being queued for remote write
...
curl -sG 'http://localhost:9090/api/v1/query' \\
     --data-urlencode 'query=prometheus_remote_storage_failed_samples_total' | jq '.data.result'`}</code></pre>

      <h3>7.5 Test AMP Data Reception</h3>
      <pre><code>{`# First, get the workspace endpoint
WORKSPACE_ID="<AMP_WORKSPACE_ID>"
AWS_REGION="<YOUR_AWS_REGION>"
...
sum(kafka_consumergroup_lag{cluster="<YOUR_CLUSTER_NAME>"}) by (consumergroup)`}</code></pre>
      ...
      <p><strong>Symptoms</strong>: No kafka metrics in Prometheus, web interface shows connection errors</p>
...
# Test authentication
kubectl exec -n monitoring -l app=kafka-exporter -- kafkacat -b <kafka-broker>:9092 -L`}</code></pre>

      <h4>Issue: ServiceMonitor Not Discovered</h4>
      <p><strong>Symptoms</strong>: Targets not showing up in Prometheus</p>
      <p><strong>Diagnostics</strong>:</p>
      <pre><code>{`# Check ServiceMonitor labels
kubectl get servicemonitor kafka-exporter -n monitoring -o yaml

# Check if Prometheus can access ServiceMonitor
kubectl get servicemonitor -A --show-labels`}</code></pre>

      <h4>Issue: Remote Write Failures</h4>
      <p><strong>Symptoms</strong>: Metrics in Prometheus but not in AMP</p>
      <p><strong>Diagnostics</strong>:</p>
      <pre><code>{`# Check IAM permissions
...
kubectl run amp-test --rm -it --restart=Never --image=curlimages/curl -- \\
  curl -v https://aps-workspaces.<YOUR_AWS_REGION>.amazonaws.com/workspaces/<AMP_WORKSPACE_ID>/api/v1/remote_write`}</code></pre>

      <h3>9.2 Useful Debugging Commands</h3>
      <pre><code>{`# List all ServiceMonitors
...
kubectl run netcheck --rm -it --restart=Never --image=busybox -- \\
  nc -zv kafka-exporter.monitoring.svc.cluster.local 9308`}</code></pre>

      <h2>10 Maintenance and Monitoring</h2>
      <h3>10.1 Regular Health Checks</h3>
      <pre><code>{`# Weekly health check script
#!/bin/bash
  ...
  - name: knative
    ...
    - alert: KnativeBrokerDown
      ...
      labels:
        severity: critical
      annotations:
        summary: "Knative broker is down"`}</code></pre>
    </DocPage>
  );
};

export default KafkaKnativeAmpSetup;
