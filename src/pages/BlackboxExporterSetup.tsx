...
const BlackboxExporterSetup = () => {
  return (
    <DocPage title="Blackbox Exporter Setup for HTTP Service Monitoring">
...
spec:
  ...
  - port: 9115
    ...
    name: http
  selector:
    app: blackbox-exporter`}</code></pre>
...
spec:
  ...
  - port: http
    ...
    - sourceLabels: [instance]
      targetLabel: service
      replacement: loki-gateway`}</code></pre>
...
# HTTP response codes
probe_http_status_code{job="loki-gateway-probe"}`}</code></pre>
...
- name: blackbox-alerts
  ...
  - alert: SSLCertificateExpiringSoon
    ...
    annotations:
      ...
      description: "SSL certificate for {{ $labels.instance }} expires in {{ $value }} days"`}</code></pre>
...
spec:
  ...
  - port: http
    ...
    - targetLabel: service
      replacement: tempo-gateway`}</code></pre>
...
curl "http://localhost:9115/probe?target=https://example.com&module=http_2xx"`}</code></pre>

      <h3>Verify Prometheus Scraping</h3>
      <pre><code>{`# Check ServiceMonitor
kubectl get servicemonitors -n monitoring

# Port forward to Prometheus and check targets
kubectl port-forward -n monitoring svc/prometheus-service 9090:9090
# Navigate to http://localhost:9090/targets`}</code></pre>
      ...
      <pre><code>{`# Service uptime percentage
...
probe_success{job=~".*-probe"}`}</code></pre>
...
export default BlackboxExporterSetup;
